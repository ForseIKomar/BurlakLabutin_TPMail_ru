...
void CChecker::WhereCanMove( std::vector</*std::shared_ptr<CCell*/SPoint/*>*/> &potential_cells )
{
	...
	for( short j = 0; j < 2; ++j ){
		...
		//if( new_cell != nullptr && new_cell -> IsEmpty() ){
			potential_cells.push_back( /*new_cell*/ SPoint( i_new_row, i_column + add[j] ) );
		//}
	}
}


void CChecker::WhereCanAttack( std::vector</*std::shared_ptr<CCell*/SPoint/*>*/> &potential_cells )
{
	short i_row  = figure_cell_ -> GetRowNumber();
	short i_column = figure_cell_ -> GetColumnNumber();
	...
	//	enemy_cell = figure_cell_ -> GetBoard() -> GetCell( i_enemy_row, i_column + add[j][0] );
		...
		//{
			potential_cells.push_back( /*move_cell*/SPoint( i_row + add[i][j], i_column + add[i][j] ) );
		//}
	}
	}
}
