...
CBoard::CBoard()
{
    ...
    for( auto &row: game_field_ ){
        for( auto &cell: row ){
            cell = std::shared_ptr<CCell>( new CCell( SPoint( i_column, i_row ) ) );
            ++i_column;
        }
        ++i_row;
    }
...
void CBoard::SetCheckers()
{
    ...
    for( short i_row = 0; i_row < cells_row_count; ++i_row ){
        for( short i_column = cell_x[ i_row % 2 ]; i_column < kBoardSideSize; i_column += 2 ){
            game_field_[i_row][i_column]->SetFigure( std::shared_ptr<CChecker>( new CChecker('B', game_field_[i_row][i_column]) ) );
        }
    }
    // расстановка белых шашек
    for( short i_row = cells_row_count + 2; i_row < kBoardSideSize; ++i_row ){
        for( short i_column = cell_x[i_row%2]; i_column < kBoardSideSize; i_column += 2 ){
           game_field_[i_row][i_column]->SetFigure( std::shared_ptr<CChecker>( new CChecker('W', game_field_[i_row][i_column]) ));
        }
    }
}
